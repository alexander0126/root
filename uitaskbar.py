...
def SaveMouseButtonSettings():
	global MOUSE_SETTINGS
	open("miles/mouse.cfg", "w").write("%s\t%s" % tuple(MOUSE_SETTINGS))

def LoadMouseButtonSettings():
	global MOUSE_SETTINGS
	tokens = open("miles/mouse.cfg", "r").read().split()
...
class TaskBar(ui.ScriptWindow):
	...
	def LoadWindow(self):
		...
		if app.FIX_BUFF_TROLL:
			toggleButtonDict[TaskBar.METIN_NAMES_PRO]=self.GetChild("MetinProNames")
		try:
