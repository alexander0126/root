...
class Interface(object):
	...
	def __init__(self):
		...
		if app.ENABLE_SYSTEM_METINSTONE:
			self.wndBonusPage = None
			self.wndFixBonusPage = 0
	...
	def __MakeTaskBar(self):
		...
		if app.FIX_BUFF_TROLL:
			self.wndTaskBar.SetToggleButtonEvent(uiTaskBar.TaskBar.METIN_NAMES_PRO, ui.__mem_func__(self.ToggleMetinNamesProDialog))
	...
	def MakeHyperlinkTooltip(self, hyperlink):
		...
		if tokens and len(tokens):
			...
			if app.ENABLE_PM_IN_GLOBAL_CHAT and "pm" == type:
				if str(player.GetName()) != str(tokens[1]):
	...
	def Close(self):
		...
		if app.ENABLE_SYSTEM_METINSTONE:
			if self.wndBonusPage:
				self.wndBonusPage.Hide()
				self.wndBonusPage.Destroy()
				self.wndFixBonusPage == 0
		...
		if app.ENABLE_SYSTEM_METINSTONE:
			del self.wndBonusPage
			del self.wndFixBonusPage
	...
	if app.FIX_BUFF_TROLL:
		def ToggleMetinNamesProDialog(self):
			chat.AppendChat(chat.CHAT_TYPE_INFO, "Nirvana GR")
	...
	if app.ENABLE_SYSTEM_METINSTONE:
		def ToggleBonusWindowPage(self):
			if self.wndFixBonusPage == 1:
				self.wndBonusPage.Hide()
				self.wndFixBonusPage = 0
			else:
				self.wndFixBonusPage = 1
				import uiBonus
				self.wndBonusPage = uiBonus.MenuDialog()
				self.wndBonusPage.Show()
	...
	def __HideWindows(self):
		...
		if app.ENABLE_SYSTEM_METINSTONE and self.wndBonusPage:
			hideWindows += self.wndBonusPage,
