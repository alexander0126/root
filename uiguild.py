...
class AcceptGuildWarDialog(ui.ScriptWindow):
	...
	if app.ENABLE_IMPROVED_GUILD_WAR_SYSTEM:
		def Open(self, guildName, warType, maxPlayer, maxScore):
			...
			self.inputValue1.SetText(str(self.maxScore) + uiScriptLocale.NEW_MAX_SCORRER_GUILD)
			self.inputValue2.SetText(str(self.maxPlayer) + uiScriptLocale.NEW_MAX_PLAYER_GUILD)
			self.SetCenterPosition()
			self.SetTop()
			self.Show()
	else:
...
class BuildGuildBuildingWindow(ui.ScriptWindow):
	GUILD_CATEGORY_LIST = (("HEADQUARTER", localeinfo.GUILD_HEADQUARTER),("FACILITY", 	localeinfo.GUILD_FACILITY),("OBJECT", 	localeinfo.GUILD_OBJECT))
	MODE_VIEW = 0
	MODE_POSITIONING = 1
	MODE_PREVIEW = 2
