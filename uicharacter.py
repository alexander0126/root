...
class CharacterWindow(ui.ScriptWindow):
	...
	def __BindObject(self):
		...
		if app.ENABLE_SYSTEM_METINSTONE:
			...
			self.statGUI.SetSize(245, 125)
			self.statGUI.SetPosition(s1+80, s2+23)
			self.statGUI.AddFlag('float')
			self.statGUI.SetTitleName(localeinfo.STATUS_TXT_KEFALIDA)
			self.statGUI.Hide()
			...
			self.statADD.SetPosition(32, 76)
			self.statADD.SetUpVisual("d:/ymir work/ui/public/xlarge_button_01.sub")
			self.statADD.SetOverVisual("d:/ymir work/ui/public/xlarge_button_02.sub")
			self.statADD.SetDownVisual("d:/ymir work/ui/public/xlarge_button_03.sub")
			self.statADD.SetText(localeinfo.STATUS_TXT_OK)
			self.statADD.Show()
	...
	if app.ENABLE_SYSTEM_METINSTONE:
		def __OverInStatMinusButton(self, stat):
			try:
				self.__ShowStatToolTip(self.STAT_MINUS_DESCRIPTION[stat])
			except KeyError:
				pass
	...
	def OnUpdate(self):
		...
		if app.ENABLE_SYSTEM_METINSTONE:
			...
			self.statGUI.SetPosition(x+30, y+13)
